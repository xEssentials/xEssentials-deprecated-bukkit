...
public class FurnaceProtectedEvent implements Listener {
	...
	public void onPlace(BlockPlaceEvent e) {
		if(e.getBlock().getType() == Material.FURNACE) {
			if(pl.getManagers().getProtectionDBManager().register(e.getPlayer().getName(), e.getBlock())) {
			...
			if(e.getClickedBlock().getType() == Material.FURNACE) {
				if(pl.getManagers().getProtectionDBManager().isRegistered(e.getClickedBlock())) {
					if(pl.getManagers().getProtectionDBManager().isOwner(e.getPlayer().getName(), e.getClickedBlock()) || e.getPlayer().hasPermission(PermissionKey.IS_ADMIN.getPermission())) {
						e.getPlayer().sendMessage(ChatColor.GREEN + "opening privated furnace of " + (pl.getManagers().getProtectionDBManager().isOwner(e.getPlayer().getName(), e.getClickedBlock()) ? "you" : pl.getManagers().getProtectionDBManager().getOwners(e.getClickedBlock())));
					} else {
