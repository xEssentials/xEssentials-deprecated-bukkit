			...
			if(e.getClickedBlock().getType() == Material.DISPENSER) {
				if(pl.getManagers().getProtectionDBManager().isRegistered(e.getClickedBlock())) {
					if(pl.getManagers().getProtectionDBManager().isOwner(e.getPlayer().getName(), e.getClickedBlock()) || e.getPlayer().hasPermission(PermissionKey.IS_ADMIN.getPermission())) {
						e.getPlayer().sendMessage(ChatColor.GREEN + "opening privated dispenser of " + (pl.getManagers().getProtectionDBManager().isOwner(e.getPlayer().getName(), e.getClickedBlock()) ? "you" : pl.getManagers().getProtectionDBManager().getOwners(e.getClickedBlock())));
					} else {
